...
begin
 ...
 StrList.Sorted:=false;   	//True - сортировать, False - не сортировать
...
begin
  SCL;
  clean_grid;
  StrList.LoadFromFile(FileOpen1.Dialog.FileName);// Загрузка текстового файла в StrList
  n:=strlist.Count;
  namef:= FileOpen1.Dialog.FileName;
  stringgrid1.RowCount:=n+1;
    readf;           // процедура считывания массива
    print_mas;      //  выводим массив в StringGrid

  MainMenu1.Items.Items[0].items[1].Enabled := true; // Разрешаем меню
...
MainMenu1.Items.Items[0].items[1].Enabled := False;     // Запрещаем меню
...
begin
showmessage('Выполнил студент факультета КБ-4 группы БСБО-01-15: '#139'Коваленко Вячеслав Андреевич'#13#10'Контактные данные: pro100ckat@mail.ru');
end;
...
begin
if MessageDlg('Выйти из программы? Все несохраненные данные будут потеряны!',
...
begin
showmessage('Программа предназначена для ведения БД по различным товарам предприятия.'#13#10'Разработана в среде Delphi 10 Seatle в 2016 году.'#13#10'Версия: Beta 1.0');
end;

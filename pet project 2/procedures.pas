...
VAR a,b : array [1..25] of tovar; 		//-массив записей
f: TextFile;	 	      // -файлы для исходных данных и результатов
var v:tovar;
io,gl:integer;
...
//ИСПРАВИТЬ СОХРАНЕНИЕ И ОТКРЫТИЕ
...
begin
 ...
 form1.StringGrid1.Cells[3,0]:='Магазины';
 form1.StringGrid1.Cells[4,0]:='Количество';
end;
...
while not eof(f) do
 ...
 s:=s+' ';
  ...
  a[i].name:=copy(s,1,pos('|',s)-1);//имя до второго пробела
  delete(s,1,pos('|',s));
  a[i].price:=copy(s,1,pos(' ',s));//имя до второго пробела
  delete(s,1,pos(' ',s));
  a[i].shops:=copy(s,1,pos('!',s)-1);//имя до второго пробела
  delete(s,1,pos('!',s));
  a[i].kolvo:=copy(s,1,pos('.',s));//имя до второго пробела
  delete(s,1,pos('.',s));
  i:=i+1;      //считаем
 end;
...
begin
  ...
  closefile(f);
  Application.MessageBox(Pchar('Файл успешно сохранен'), 'Сохранение...' );
end;
...
begin
    ...
    a[n].kolvo:=form2.edit4.Text+' кг.' else
    a[n].kolvo:=form2.edit4.Text+' шт.';
   Application.MessageBox(Pchar('Товар успешно добавлен'), 'Добавление завершено' );
...
var index,i:integer;
begin
    if MessageDlg('Уверены, что хотите удалить эту запись?',
            mtConfirmation, [mbYes, mbNo], 0) = mrYes
            then begin
            index:=ydal.combobox1.ItemIndex+1;
    if (index<=n) then begin

  For i:= index to n-1 do { kol - общее количество студентов}
   Begin
   a[i].name:=a[i+1].name;
   ...
   n:=n-1;   {После сдвига уменьшаем количество студентов на 1}
   form1.StringGrid1.RowCount:=form1.stringgrid1.RowCount-1;
    end;
Application.MessageBox(Pchar('Удаление записи завершено!'), 'Удаление...' );
    end
    else
   Application.MessageBox(Pchar('Товара под этим номером нет в списке'), 'Ошибка' );
end;
